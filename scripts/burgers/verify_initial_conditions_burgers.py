...
initial_conditions_domain = torch.tensor([[0, -1], [0, 1]], dtype=dtype)

def empirical_evaluation(model, grid_points):
    return model(grid_points) - (-torch.sin(torch.pi * grid_points[:, 1].unsqueeze(1)))
...
def crown_verifier_function(layers, piece_domain, debug=True):
    ...
    ub, lb = burgers.compute_sin_initial_conditions_bounds(
        piece_domain,
        debug=debug
    )
